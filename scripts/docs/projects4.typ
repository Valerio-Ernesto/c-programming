...
== Chapter 4 Expressions@cProgramming4
...
+ Write a program that asks the user to enter a two-digit number, then prints the number with
  ...
  Read the number using `%d`, then break it into two digits. _Hints:_ It `n` is an integer, then `n % 10`
  is the last digit in `n` and `n / 10` is `n` with the last digit removed.

  #underline("Answer"): #text(fill:olive)[#link("files/chapter/src/")[Source Code]]
...
+ Rewrite the program in Programming Project 2 so that it prints the reversal of a three-digit
  number without using arithmetic to split the number into digits. _Hint:_ See the `upc.c` program
  of Section 4.1.
...
+ Write a program that reads an integer entered by the user and displays it in octal (base 8):
  ...
  The output should be displayed using five digits, even if fewer digits are sufficient. _Hint:_ To 
...
+ European countries use a 13-digit code known as a European Article Number (EAN)
  ...
  the `upc.c` program of Section 4.1 so the it calculates the check digit for an EAN. The 
